...
- **WSJTX bridge integration is work-in-progress.** PTT now works (fixed 2026-06-24: the built frontend bundle had `args === 1 || args === true` instead of `Number(args) > 0`, causing Hamlib PTT type 3 / `RIG_PTT_RIG` to evaluate as `false`). Remaining work includes: the bridge helper is manually launched (not yet spawned/managed by the server); audio routing between WSJTX and RigControl Web via the PipeWire virtual devices is untested end-to-end; the browser sends `sendResult(true)` to the bridge immediately (fire-and-forget) without waiting for server confirmation — a failed `set-ptt` on the server side is reported as success to WSJTX; `--debug-wsjtx` diagnostic logging added to `server/rigComm.ts` (`set-ptt` and `set-frequency` handlers) and `useWsjtxBridge.ts` (PTT confirmation tracking, `rig-error` listener). The bridge helper's `--verbose` flag now includes timestamps and PTT-specific tracing. **Important: after modifying `useWsjtxBridge.ts` or any frontend source, run `npm run build` to rebuild the Vite bundle — the Electron app serves the pre-built `dist/` output, not live source.**
