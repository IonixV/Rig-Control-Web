...
- **`customInit`** — Migrates FT4222 DLLs (`ftd2xx.dll`, `LibFT4222-64.dll`) from the old install directory (`$INSTDIR`) to `%LOCALAPPDATA%\RIGCONTROL WEB\` before the installer wipes the old directory during an upgrade. Only copies files that exist in `$INSTDIR` and don't already exist in the target. This ensures FT-710 spectrum scope users don't have to re-copy the DLLs after every update.
